...
# Plastic card types
plastic_cards = [
    "Visa Classic", "Visa Gold", "Visa Platinum", "MasterCard Standard",
    "MasterCard Gold", "MasterCard Platinum", "American Express",
    "Discover", "UnionPay", "Maestro", "Visa Electron", "JCB",
    "Diners Club", "Prepaid Card", "Business Card", "Salary Card"
]

# Define column order explicitly to match JSON structure
fieldnames = [
    "CustomerID", "FirstName", "LastName", "Gender", "Email", "Phone", "Age", 
    "City", "Country", "CurrentBalance", "Currency", "TotalTransactions", 
    "TotalDeposits", "TotalWithdrawals", "AverageTransactionAmount", "TotalCashback", 
    "LastTransactionDate", "PreferredContactMethod", "AverageMonthlySpending", 
    "HighestTransactionAmount", "LowestTransactionAmount", "TotalNumberOfAccounts", 
    "AccountStatus", "RiskProfile", "DepositStatus", "LoanStatus", 
    "InternationalTransactionIndicator", "VATUserStatus", "TotalVATRefundAmount", 
    "DeviceModel", "AppVersion", "RecentActivityFlag", "PreferredLanguage", 
    "Delivery", "PlasticCard"
]
...
for i in range(100):
    ...
    customer_dict = {
        "CustomerID": first_name + last_name + str(i),
        "FirstName": first_name,
        "LastName": last_name,
        "Gender": gender,
        "Email": fake.email(),
        ...
        "Country": "United States",
        "CurrentBalance": round(random.uniform(100, 50000), 2),
        "Currency": random.choice(["USD", "EUR"]),
        "TotalTransactions": random.randint(10, 1000),
        "TotalDeposits": round(random.uniform(1000, 50000), 2),
        "TotalWithdrawals": round(random.uniform(500, 45000), 2),
        "AverageTransactionAmount": round(random.uniform(10, 1000), 2),
        "TotalCashback": round(random.uniform(10, 500), 2),
        "LastTransactionDate": fake.date_between(start_date='-1y', end_date='today').strftime('%Y-%m-%d'),
        "PreferredContactMethod": random.choice(["Email", "Phone", "SMS"]),
        "AverageMonthlySpending": round(random.uniform(100, 5000), 2),
        "HighestTransactionAmount": round(random.uniform(500, 10000), 2),
        "LowestTransactionAmount": round(random.uniform(1, 50), 2),
        "TotalNumberOfAccounts": random.randint(1, 5),
        "AccountStatus": random.choice(["Active", "Dormant"]),
        "RiskProfile": random.choice(["Low", "Medium", "High"]),  # ✅ Fixed RiskProfile placement
        "DepositStatus": random.choice(["Yes", "No"]),
        "LoanStatus": random.choice(["Yes", "No"]),
        "InternationalTransactionIndicator": random.randint(0, 500),
        "VATUserStatus": random.choice(["Yes", "No"]),
        "TotalVATRefundAmount": round(random.uniform(10, 1000), 2),
        "DeviceModel": random.choice(["iPhone 13", "Samsung Galaxy S21", "MI 8", "Redmi Note 7", "Pixel 8"]),
        "AppVersion": f"v{random.randint(1, 5)}.{random.randint(0, 9)}",
        "RecentActivityFlag": random.choice(["Yes", "No"]),
        "PreferredLanguage": random.choice(["English", "Spanish", "French"]),
        "Delivery": round(random.uniform(10, 1000), 2),
        "PlasticCard": random.choice(plastic_cards)
    }
    customers.append(customer_dict)
...
csv_filename = "customer_profile_data.csv"
with open(csv_filename, 'w', newline='') as csv_file:
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)  # ✅ Ensures exact column order
    writer.writeheader()
    writer.writerows(customers)

print(f"✅ Data saved to {csv_filename}")
print("First 2 records:", customers[:2])
